...
class knn_cl():
    ...
    def __init__(self, kg, data_process):
        ...
        self.negative_lab_size = self.batch_size
    ...
    def get_k_means_test(self):
        ...
        for i in range(6):
            ...
            test = [self.test_data[j] for j in index]
            self.test_group.append(test)

    def get_k_means_train_death(self):
        self.length_train_death = len(self.train_death_data_reduced)
        self.train_death_matrix = np.zeros((self.length_train_death, self.lab_size+self.item_size))
    ...
    def get_k_mean_test_non_death(self):
        ...
        self.test_non_death_center = self.kmeans_test_non_death.cluster_centers_

        self.test_non_death_group = []
        for i in range(3):
            index = np.where(self.kmeans_test_non_death.labels_==i)[0]
            test = [self.test_non_death_data_reduced[j] for j in index]
            self.test_non_death_group.append(test)
    ...
    def get_k_mean_test_death(self):
        ...
        for i in range(self.length_test_death):
        ...
        self.test_death_center = self.kmeans_test_death.cluster_centers_

        self.test_death_group = []
        for i in range(3):
            index = np.where(self.kmeans_test_death.labels_==i)[0]
            test = [self.test_death_data_reduced[j] for j in index]
            self.test_death_group.append(test)
        ...
        for j in range(self.epoch):
            ...
            for i in range(iteration):
                if j == 0:
                    ...
                    self.err_ = self.sess.run([self.cross_entropy, self.train_step_combine_cl],
                                              feed_dict={self.input_x_vital: self.train_one_batch_vital,
                                                         self.input_x_lab: self.train_one_batch_lab,
                                                         self.input_x_demo: self.train_one_batch_demo,
                                                         # self.input_x_com: self.one_batch_com,
